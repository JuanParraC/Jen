...
namespace Jen.Json
{
    ...
    public class Arreglo : List<object>
    {
        ...
        {
            StringBuilder sb = new StringBuilder(this.Count * Json.TotalByteXEntrada);
            Util.SerializarObjeto(ref sb, this);
            sb.Length--;
            return sb.ToString();
        }
    }
    ...
    public static class Json
    {
        ...
        public static object Evaluar(string json)
        {
            if (!string.IsNullOrEmpty(json))
            {
                int indice = 0;
                bool exito = true;
                return compilar(json, ref indice, ref exito);
            }
            return null;
        }
        // procesa Objeto
        static Objeto objeto(string json, ref int indice)
        {
            ...
            while (!salir)
            {
                ...
                else
                {
                    ...
                    bool exito = true;
                    object val = compilar(json, ref indice, ref exito);
                    if (!exito)
                        return null;
                    //ret.Add(prop, val);
                    ret[prop] = val;
        ...
        static Arreglo arr(string json, ref int indice)
        {
            ...
            while (!salir)
            {
                ...
                {
                    bool exito = true;
                    object o = compilar(json, ref indice, ref exito);
                    if (!exito)
                    {
                        return null;
                    }
        ...
        // compila un objeto
        static object compilar(string json, ref int indice, ref bool exito)
        {
        ...
        {
            StringBuilder sbTexto = new StringBuilder(20);
            ...
            while (!completado)
            {
                ...
                else if (chActual == CaracteresJson.backSlashBackSlash)
                {
                    if (indice == json.Length)
                    {
                        break;
                    ...
                    {
                        sbTexto.Append(chActual);
                    }
                    else if (chActual == CaracteresJson.b)
                    {
                        sbTexto.Append(CaracteresJson.backSlashB);
                    }
                    else if (chActual == CaracteresJson.f)
                    {
                        sbTexto.Append(CaracteresJson.backSlashF);
                    }
                    else if (chActual == CaracteresJson.n)
                    {
                        sbTexto.Append(CaracteresJson.backSlashN);
                    }
                    else if (chActual == CaracteresJson.r)
                    {
                        sbTexto.Append(CaracteresJson.backSlashR);
                    }
                    else if (chActual == CaracteresJson.t)
                    {
                        sbTexto.Append(CaracteresJson.backSlashT);
                    }
                    else if (chActual == CaracteresJson.u)
                    {
                        ...
                        if (largoRestante >= 4)
                        {
                            //char[] arregloCharUnicode = new char[4];
                            //Array.Copy(json, indice, arregloCharUnicode, 0, 4);
                            // parsea un hexadecimal de 32 bit en un entero
                            string sNum = json.Substring(indice, 4);
                            //int car = int.Parse(new string(arregloCharUnicode), NumberStyles.HexNumber, Util.Cultura);
